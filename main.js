...
pointLight.position.set(5, 15, 30);
...
loader.load( 'me3d test.glb', function ( gltf ) {
...
let glasses;
loader.load( 'glasses.glb', function ( glas ) {
...
function loadUm() {
    loader.load('umActually.v1.U.glb', function (actually) {
...
let sign;
fbxLoader.load( 'A&ASigns.fbx', function ( fbx ) {
...
// animation for continuous looping animations
...
function progressCircle(degrees){
    ...
    else if (200 <= degrees && degrees < 240) {
        fPlace3d = (degrees - 200) * 100 / 4000;
        if (!vids) {loadVids();}
        downFromStart(fPlace3d);
    }
    else if (240 <= degrees && degrees < 280) {
        gPlaceStory = (degrees - 240) * 100 / 4000;
    }
    else if (280 <= degrees && degrees < 320) {
        hPlace3d = (degrees - 280) * 100 / 4000;
    }
    else if (320 <= degrees && degrees < 360) {
        iPlaceBottom = (degrees - 320) * 100 / 4000;
    }
